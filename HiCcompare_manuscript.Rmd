...
The number of differences detected by HiCcompare at 5KB resolution (supplemental table 6.2) was much lower than the number FIND detected (~150,000) [@Djekidel:2018aa]. The drop off of the number of differential interactions detected by HiCcompare can be explain by the sparsity of the data at 5KB resolution. Additionally, the large number of differences detected by FIND at 5KB resolution is questionable given that he maximum resolution of the K562 and GM12878 data was found to be ~39KB and ~9KB, respectively (Supplemental table 6.1).

The differentially interaction regions detect by HiCcompare were intersected with gene locations and a pathway enrichment analysis was performed. 
...
As an additional comparison of our method with FIND we used replicate data for GM12878 data with a priori controlled changes added (see methods) and input them into both HiCcompare and FIND. HiCcompare successfully detected the majority of the controlled changes while FIND performed poorly. See supplemental tables 6.4 - 6.6 for the results of this comparison. Additionally we found that the run time of FIND on Hi-C matrices at 100KB resolution or higher was extremely long (>72 hours) even when run in parallel using 16 cores of our cluster while HiCcompare is able to complete an analysis in a matter of minutes for data at resolutions between 100KB and 50KB (supplemental figure 1.1).
