...
## 2.4 Analysis of K562 and GM12878 using HiCcompare

As an example for the usage of HiCcompare we performed a comparative analysis between Hi-C data from K562 and GM12878 cells lines. The data was obtained from GEO (GSE63525) generated by Rao & Huntley. The maximum resolution of each Hi-C matrix was calculated using the `calculate_map_resolution.sh` function from `Juicer` [@Durand:2016aa]. The maximum resolution for each file is displayed in supplemental table 6.1. Breifly, two replicates for each cell line were obtained (see data access) and the replicate contact matrices were combined for the HiCcompare analysis. HiCcompare was used to jointly normalize the data between the cell lines and then detect differences. HiCcompare analyses were performed at 1MB, 100KB, 50KB, and 10KB resolution. Differentially interaction regions were then intersected with gene locations and a pathway enrichment analysis was performed. 


<!-- put table of number of diffs here -->

The enrichment analysis showed that many of the differential regions contained genes involved in the immune system. Table XX displays the results of the enrichment analysis. We also found that the enrichment analyses at each resolution were relatively consistent further indicating the strength of HiCcompare at detecting biologically relevent differences.

## 2.5 Comparison with FIND

FIND uses a spatial Poisson process to detect differences between two Hi-C experimental conditions [@Dowen:2014aa]. FIND was intended for high resolution Hi-C data and treats interactions as spatially dependent on surrounding interactions. To compare our method with FIND we used replicate data for GM12878 data with a priori controlled changes added (see methods) and input them into both HiCcompare and FIND. HiCcompare successfully detected the maajority of the controlled changes while FIND performed poorly. See Table XX for the results of the comparison. Additionally we compared the run time of HiCcompare with FIND and found that HiCcompare is able to normalize and detect differences in a Hi-C matrix much quicker than FIND. 

Additonally we compared our analysis of K562 and GM12878 with that of the analysis performed in the FIND paper. HiCcompare detected X differences while FIND detected Y differences. The results of the HiCcompare and FIND gene enrichment analysis both found pathways related to the immune system suggesting that both methods can detect biologically significant differences. FIND performed their analysis at a resolution of 5KB, but this is questionable given that the maximum resolution of the K562 and GM12878 data was found to be ~39 KB and ~9 KB, respectively (Table XX). The data may have been too sparse for FIND to make any meaningful comparisons. 


## 2.6 Comparison with diffHiC
...
## 2.7 Preservation of A/B compartments
