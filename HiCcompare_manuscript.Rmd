...
^\*^ To whom correspondence should be addressed: mikhail.dozmorov@vcuhealth.org [!!! Need complete contact info]

Running title: HiCcompare

Keywords: Hi-C, Chromosome conformation capture, Sequencing, R
...
Discovery of biases in Hi-C data led to the development of numerous methods for normalizing _individual_ datasets [@Lieberman-Aiden:2009aa; @Imakaev:2012aa; @knight2012fast; @Cournac:2012aa]. Although normalization of individual datasets improves reproducibility of replicated Hi-C data [@Imakaev:2012aa; @Yaffe:2011aa], these methods do not explicitly account for biases between _multiple_ Hi-C datasets. The between-dataset biases are particularly problematic when comparing Hi-C datasets between biological conditions (Supplemental Section 3). When the detection of chromatin interaction differences due to biology, not biases, is important, normalization that removes the between-dataset biases is critical.  
...
The enrichment analysis showed that many of the differential regions contained genes involved in the immune system (Table 2). We also found that the enrichment analyses at each resolution were relatively consistent further indicating the strength of HiCcompare at detecting biologically relevant differences. The results of the HiCcompare and FIND gene enrichment analysis both found pathways related to the immune system suggesting that both methods can detect biologically significant differences.

**Table 2. Gene enrichment results for HiCcompare analyses.** KEGG 2016 pathways and their corresponding q-values for the enrichment analyses of the 1MB, 100KB, and 50KB differentially interacting regions. Differentially interacting regions detected by HiCcompare were intersected with a list of all gene locations and the enriched genes were then entered into a KEGG pathway enrichment analysis.
